...
Both failures share a root cause: the loop had no check it could run. Fix that, bound the turns,
and you get a loop you can walk away from.
...
**Premature completion** is the expensive one, because it looks like success. That is what a `Stop`
hook is for: it re-runs the real check after Claude decides it is finished, and exit code 2 sends
Claude back to work with the failure text. Hooks are enforced; a prompt asking Claude to leave the
tests alone is advice it may drop under pressure.
...
file edits and common filesystem commands, **not** `npm test`. The fix actually landed (`git diff`
shows the guard in `src/math.js`) — but the loop never saw green and died on the limit.

The headless page names `result`, `session_id`, `total_cost_usd`, `structured_output` and
`permission_denials`. `num_turns`, `subtype`, `is_error` and `terminal_reason` appear live but are
not named on that page — ⚠️ Needs verification before you build tooling on them; the exit code is
the safe signal.
...
`# Output may vary` — `result` trimmed. Exit code `0`. One extra flag turned `error_max_turns`
into `success`.

**Step 4: A `Stop` gate against early victory**

Break **two** things (`add` returns `a - b`, `divide` still does not throw), then install the gate:
...
Claude fixed `add`, tried to stop, the gate exited 2 with the remaining failure, and kept working —
and said so in its own words. Note the second half: it weighed deleting the test and rejected it.
That is a decision you do not want left to a prompt.
...
not a plan: if a task needs 30 turns, raise the limit.
</details>
...
**Instructions**: install the Step 4 gate, break two things, ask Claude to fix one. Then remove the
hook and run the same prompt again.

**Expected result**: with the gate, Claude keeps going and lands green; without it, it stops after
the one fix you named.
...
500 lines into a blocked turn costs context and makes the next pass worse. And keep the
`stop_hook_active` escape — without it, a check that can never go green blocks forever.
</details>
...
| `stop_hook_active` | Hook input flag: already blocked | exit 0 to release |
...
| A `Stop` hook with no `stop_hook_active` escape | A check that can never pass blocks every turn |
...
time, conventions drifted, and two developers spent three days on it. Worse, a few endpoints were
marked done on a summary alone — nobody had run the test.

**Solution**: One headless run per endpoint from a shell loop, with the cycle spelled out: schema,
resolver, update the test, run the test, move on only when it passes — at most three fix attempts,
then report for review. The test command was pre-authorized with `--allowedTools`, turns capped
with `--max-turns`, and a `Stop` hook re-ran the suite so "done" had to mean green.

**Result**: Most endpoints converged unattended, in an afternoon. The handful that did not were
reported rather than silently declared finished.
