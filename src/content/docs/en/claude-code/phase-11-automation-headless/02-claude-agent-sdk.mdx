...
would have run in your shell. `--permission-mode default` forces the stock behavior; on Pro, Max
and Team plans the built-in starting mode is `auto`, which `permissions.defaultMode` overrides.
