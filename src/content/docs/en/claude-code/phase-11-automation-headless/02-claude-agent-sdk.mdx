...
packages `@anthropic-ai/claude-code` (npm) and `claude-code-sdk` (pip) are retired. And it is not
...
Options used here (TypeScript names; Python uses `snake_case`):
...
| `permissionMode` | `'default' \| 'acceptEdits' \| 'plan' \| 'dontAsk' \| 'bypassPermissions' \| 'auto'` | Baseline for unresolved calls. `dontAsk` denies; `bypassPermissions` approves everything. |
...
| `settingSources` | `('user' \| 'project' \| 'local')[]` | Settings files to load. Omitted = all (like the CLI); `[]` = isolated. |
...
The package bundles a Claude Code binary; the SDK version tracks it. Set `ANTHROPIC_API_KEY` in
the process environment — the SDK does not read `.env` files.
...
[tool] Grep { pattern: '^\\s*(export\\s+…', path: '/Users/luatnq/cc-lab/src/math.js', output_mode: 'content' }
[success] turns=2 cost=$0.4578
...
Claude used `Grep`, not in `allowedTools` — read-only calls need no approval. And $0.46 for a
two-line file? Without `settingSources` the SDK loaded this machine's `~/.claude` (plugins,
CLAUDE.md, MCP servers). With `settingSources: []` the same script cost `$0.0873`. Every script
below sets it.

**Step 3: Let it edit — `acceptEdits`**
...
The handler runs inside your Node process. `ToolSearch` is tool search (on by default) loading
the deferred schema.
...
async def main():
    ...
    async for message in query(prompt="List the exported functions in src/math.js", options=options):
        if isinstance(message, AssistantMessage):
            ...
            print(f"[{message.subtype}] turns={message.num_turns} cost=${message.total_cost_usd:.4f}")
...
[tool] Read {'file_path': '/Users/luatnq/cc-lab/src/math.js'}
[success] turns=2 cost=$0.0890
`src/math.js` exports two functions:
...
shell string built from the prompt), and `--output-format json` plus `--json-schema` for output.
...
into a denial listed in `permission_denials`, instead of hanging on a prompt nobody answers.
...
[success] denied=0
The diff adds two functions to `src/math.js`:
…
- **`average([])` returns `NaN`** — `reduce` with initial `0` yields `0`, then `0 / 0` is `NaN`.
- **`average` with `null`/`undefined` argument throws a `TypeError`** …
- **No tests added** — neither new function has coverage …
```
...
**Goal**: Process N files, max 2 sessions at a time, 3 attempts with exponential backoff.

**Instructions**:
1. `runOnce(prompt)` runs one `query()` and throws unless `result.subtype === 'success'`.
2. `withRetry(prompt, retries = 3)` with 1s / 2s / 4s backoff.
...
if `is_error` is true or `structured_output` is missing.
...
| Expecting Claude Code's system prompt / CLAUDE.md by default | `systemPrompt: { type: 'preset', preset: 'claude_code' }`; `settingSources` with `'project'` |
...
**Scenario**: A fintech team in Ho Chi Minh City runs an internal PR-review service: every pull
request on their Kotlin backend gets an automated first pass before a human reviewer.
...
the repo's CLAUDE.md applies and nothing from the runner's home directory leaks in. Findings come
back through `outputFormat` as JSON the GitHub bot posts directly. Two in-process tools,
`get_ci_status` and `get_ticket`, replaced a pile of prompt text; the team kept the tool count small
and wrote each description as if onboarding a new hire (S9). As in Anthropic's C-compiler write-up,
tools write the full CI log to a file and return only the failing lines in context (S14).

**Result**: No shell-injection surface, schema-validated output, per-PR cost visible in
`total_cost_usd`. No published accuracy numbers; what changed is that reviewers read the bot's
comments again.
