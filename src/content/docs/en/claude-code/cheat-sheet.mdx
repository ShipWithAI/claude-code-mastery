...
| `/exit` or `Ctrl+C` twice (or `Ctrl+D`) | Exit session | Done working |
...
| Shortcut | Action |
|----------|--------|
| `Escape` | Interrupt the current turn — session and context stay intact |
| `Escape` × 2 | Rewind — show message selector to undo |
| `Ctrl+C` twice (or `Ctrl+D`) | Exit the session |
