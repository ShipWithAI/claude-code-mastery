...
Hai cái bẫy tên gọi. Tháng 9/2025, "Claude Code SDK" đổi tên thành **Claude Agent SDK**; package
...
Các option dùng trong module này (tên TypeScript; Python dùng `snake_case`):
...
| `permissionMode` | `'default' \| 'acceptEdits' \| 'plan' \| 'dontAsk' \| 'bypassPermissions' \| 'auto'` | Mức nền cho call chưa được rule nào quyết. `dontAsk` từ chối; `bypassPermissions` duyệt tất cả. |
...
| `settingSources` | `('user' \| 'project' \| 'local')[]` | File settings nào được nạp. Bỏ trống = tất cả (như CLI); `[]` = cô lập. |
...
Package đi kèm binary Claude Code; version SDK bám theo version binary. Đặt `ANTHROPIC_API_KEY`
trong environment của process — SDK không tự đọc file `.env`.
...
Claude dùng `Grep`, không nằm trong `allowedTools` — call chỉ đọc không cần duyệt. Và $0.46 cho
một file hai dòng? Không có `settingSources`, SDK nạp cả `~/.claude` của máy này (plugin,
CLAUDE.md, MCP server). Cùng script đó với `settingSources: []` chỉ tốn `$0.0873`. Mọi script
bên dưới đều đặt nó.
...
ghép prompt vào chuỗi shell), và `--output-format json` cộng `--json-schema` cho output.
...
**Mục tiêu**: Xử lý N file, tối đa 2 session cùng lúc, 3 lần thử với exponential backoff.

**Hướng dẫn**:
1. `runOnce(prompt)` chạy một `query()` và throw nếu `result.subtype !== 'success'`.
2. `withRetry(prompt, retries = 3)` với backoff 1s / 2s / 4s.
...
`is_error` là true hoặc thiếu `structured_output`.
...
| `for await (const m of query({ prompt, options }))` | `async for m in query(prompt=…, options=…)` | Async iterator |
...
| Tưởng mặc định có system prompt / CLAUDE.md của Claude Code | `systemPrompt: { type: 'preset', preset: 'claude_code' }`; `settingSources` có `'project'` |
...
CLAUDE.md của repo có hiệu lực và không có gì từ home directory của runner lọt vào. Kết quả trả
về qua `outputFormat` dạng JSON để bot GitHub đăng thẳng. Hai in-process tool, `get_ci_status` và
`get_ticket`, thay cho cả đống prompt text; team giữ số tool ít và viết mô tả từng tool như đang
onboard new hire (S9). Như bài viết C compiler của Anthropic, tool ghi toàn bộ CI log ra file và
chỉ trả vài dòng lỗi vào context (S14).

**Kết quả**: Không còn bề mặt shell injection, output đã validate theo schema, chi phí mỗi PR đọc
được từ `total_cost_usd`. Team không công bố con số độ chính xác; điều thay đổi là reviewer đọc
lại comment của bot.
