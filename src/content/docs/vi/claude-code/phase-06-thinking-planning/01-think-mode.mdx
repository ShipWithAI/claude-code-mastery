...
harder" để mở chế độ sâu hơn. Những câu đó **không làm gì cả**: Claude Code "passes other
phrases such as 'think', 'think hard', and 'think more' through as ordinary prompt text and
doesn't recognize them as keywords".

Nghĩa là bạn trả tiền cho câu thần chú, còn hai thứ xoay được núm — **effort level** và
keyword **`ultrathink`** — thì nằm không. Tệ hơn: extended thinking đã bật sẵn từ đầu. Module
này thay folklore bằng bốn công tắc thật.

---

## 2. CONCEPT — Ý Tưởng Cốt Lõi

**Extended thinking bật mặc định.** Không có "mode" nào để vào. Bạn điều khiển: Claude suy
luận *nhiều bao nhiêu* (effort level, cả session), một lượt có được thêm không (`ultrathink`),
có suy luận hay không (`Option+T`), và bạn có thấy nó không (`Ctrl+O`).

Effort level là núm chính, và docs gán cho mỗi level một việc. Đây chính là ma trận
think-vs-act cũ, đổi trục sang những level có thật:

| Task thuộc dạng… | Effort |
|---|---|
| "short, scoped, latency-sensitive… not intelligence-sensitive" | `low` |
| nhạy chi phí, "can trade off some intelligence" | `medium` |
| code hằng ngày: "balances token usage and intelligence" | `high`, mặc định |
| đáng "deeper reasoning at higher token spend" | `xhigh` |
| nặng, nhưng "prone to overthinking" — "test before adopting broadly" | `max` |
| đủ lớn để điều phối: "`xhigh` … cộng dynamic workflows" | `ultracode` |

`high` là "the default on every model except Opus 4.7" — model đó mặc định `xhigh`.

`ultrathink` là keyword duy nhất Claude Code nhận ra, chỉ tác dụng một lượt: "Include
...
Mọi bước chạy trong lab repo có `src/math.js` và một file test.

**Bước 1: Đọc effort level đang dùng**

Chạy `claude` và nhìn ngay trên ô prompt. {/* docs: model-config */}
...
Slider hiện các level mà model này có — Opus 4.6 và Sonnet 4.6 không có `xhigh`.
`/effort <level>` đặt thẳng; `/effort auto` xóa level đã lưu cho model đó.
...
Enabled đã tick sẵn. `/config` lưu nó thành `alwaysThinkingEnabled` trong
...
`Thought for 2s` là tất cả bạn thấy: "Claude Code collapses thinking output by default."
...
Cả hai lượt chỉ đọc file nên `--allowedTools "Read"` là toàn bộ quyền cần cấp. Hai object
JSON cùng prompt, cùng repo: `result` 1.399 ký tự ở `low` so với 2.494 ở `high`; `num_turns`
2 so với 4; `duration_ms` 10.615 so với 40.056. Output may vary — một cặp run trên một máy.
So độ dài, số turn và nội dung, **đừng bao giờ** so token count của thinking: Claude Code
không in con số đó.
...
Với task cơ học, `low` thường hòa `high`, nên ghim theo model bằng `modelSettings`. Docs nói
rất rõ: "set `effortLevel` to `low`, `medium`, `high`, or `xhigh` as the default for models
without one. `max` isn't accepted as a level in either key, and `ultracode` has its own
`ultracode` key." Vậy `max` phải đến từ `/effort` hoặc `--effort`.
</details>
...
|---|---|---|
| `/effort` · `/effort auto` | Session | Mở slider (`s` = chỉ session này) · xóa level đã lưu |
| `/effort <level>`, `--effort <level>` | Session, CLI | `low`, `medium`, `high`, `xhigh`, `max`, `ultracode` |
| `effortLevel`, `modelSettings` | settings.json | Default, và theo từng model: chỉ `low`–`xhigh` |
| `ultracode` | settings.json | Key riêng — `xhigh` cộng dynamic workflows |
...
| `"effortLevel": "max"` trong settings.json | "`max` isn't accepted as a level in either key". Dùng `/effort max` hoặc `--effort max` |
