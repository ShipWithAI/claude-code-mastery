...
| `/exit` hoặc `Ctrl+C` hai lần (hoặc `Ctrl+D`) | Thoát phiên | Xong việc |
...
| Phím tắt | Chức năng |
|----------|-----------|
| `Escape` | Ngắt turn hiện tại — session và context vẫn giữ nguyên |
| `Escape` × 2 | Rewind — hiện danh sách message để undo |
| `Ctrl+C` hai lần (hoặc `Ctrl+D`) | Thoát session |
