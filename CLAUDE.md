...
- **Digital course** (Markdown → Astro Starlight website)
...
**Languages**: English (`src/content/docs/en/claude-code/`) and Vietnamese
(`src/content/docs/vi/claude-code/`) — always maintain both in sync.
...
- **16 Phases** → **64 Modules** → **~200+ Topics**
...
Every module MUST follow the **7-Block Structure** exactly. No exceptions. This ensures consistency across all 64 modules, both languages, and all output formats.
...
### Seven principles (audit 2026-09)
1. **Keep the mental model, re-platform the mechanics.** If the concept is right, keep it; only
   replace commands, flags, config files and outputs with the native 2026 feature.
2. **Enforced > recommended.** When teaching control, prefer `permissions.allow/deny/ask`,
   `PreToolUse` hooks with exit 2, sandbox, managed settings. CLAUDE.md and prompts are advisory
   only — say so explicitly.
3. **Headless must pre-authorize.** Every `claude -p` that writes files or runs commands needs
   `--permission-mode acceptEdits` / `--allowedTools "Edit,Write"` / or
   `--dangerously-skip-permissions` **with** a note that it is sandbox-only.
4. **Esc ≠ Ctrl+C; `/context` ≠ `/cost`.** Esc interrupts the turn and keeps the session;
   Ctrl+C ×2 exits. `/context` shows occupancy; `/cost` shows spend.
5. **No fakes.** No hand-written output, no unsourced performance numbers, no invented
   repos/star counts, no recurring character ("Susan" ×9).
6. **EN + VI in the same task, same PR.** Never merge EN while the matching VI still carries
   the old defect.
7. **Teach the way Anthropic works.** Anchor every major practice in the course to an official
   Anthropic source (best-practices docs, engineering blog, AI-native SDLC playbook) with URL +
   date; use only numbers Anthropic has published, never invented or third-party figures.

When claiming "Anthropic recommends/does X", cite `docs/references/anthropic-sources.md` by ID
(e.g. `(S1)`).
...
- [ ] Frontmatter has `verified: YYYY-MM-DD` and `claude_version: X.Y.Z`
- [ ] `npm run lint:course` reports no errors for this module
- [ ] Every `claude -p` that writes files or runs commands carries a permission flag
- [ ] Anthropic practice (if any) cites `docs/references/anthropic-sources.md` by registry ID

### Definition of Done (per module, EN and VI)
- 7 blocks in order; H2 = 7; no `##` inside code blocks
- Frontmatter has `title`, `description`, `verified: YYYY-MM-DD`, `claude_version: X.Y.Z`
- Word count 800–1500 (VI may run ~10% longer); hard max 2200
- Every command/flag/setting key/hook event/frontmatter field traceable to a docs page named in
  the PR; no blacklist pattern left
- Every "Expected output" was actually run and carries `# Output may vary`; model prices keep
  `⚠️ Needs verification` unless checked against the pricing page
- Every `claude -p` that writes files or runs commands has a permission flag
- At least one exercise has a `<details>` ✅ Solution
- No unsourced performance numbers; no recurring "Susan" character except one kept case
- Cross-references use the right module number/slug; "Next" link correct
- If `docs/references/anthropic-sources.md` maps a practice to this module, it is woven into
  the right block with the registry ID
- VI is parallel authoring in the same PR; technical terms stay English
- `npm run lint:course` and `npm run build` green
...
5. **Diagrams**: use Mermaid (rendered by the Starlight site). No exported diagram PNGs are kept in the repo.
6. **Screenshots**: store under `public/` with descriptive names like `02-permission-prompt.png`.
